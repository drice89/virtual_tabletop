    ...
    return (
      <ul className={`${active ? pieceStyles.active : pieceStyles.hidden} ${pieceStyles.piecesList}`}>
        {Object.values(pieces).map((piece) => (
          ...
          <li key={piece._id} className={`${pieceStyles.singlePiece}`} onMouseOver={() => this.setState({ showDelete: piece._id })} onMouseOut={() => this.setState({ showDelete: null })}>
            {' '}
            <span onClick={this.handleDelete(piece._id)} className={`${pieceStyles.deletePieceButton}  ${this.state.showDelete === piece._id ? pieceStyles.showDelete : null}`}>&#x292C;</span>
